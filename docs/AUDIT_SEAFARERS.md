...
- **公式ルール未実装（うち一部はコードコメントで簡略を明言）**:
  ...
  - [D2] **S7 海賊の島々**で 盗賊・最長交易路・最大騎士力 が無効化されていない（公式は3つとも不使用）。軍船・艦隊戦闘も未実装（簡略を明言）。
...
| 建物隣接停止で攻撃 | ⚠️ | 停止頂点の建物所有者から**無条件ランダム1枚強奪**（pirateIslands.ts:54-71）。戦力比較なし→次行 |
| **戦闘判定（海賊強さ=目／自分強さ=軍船数）** | ❌ | 未実装。常にランダム1枚のみ。コメントで簡略明記（pirateIslands.ts:9、scenarios.ts:562） |
| **軍船化（騎士で通常船を軍船化）** | ❌ | 未実装（`warship`概念がコードに無い） |
| **盗賊 不使用** | ❌ | S7 が `useRobber:false` 未設定（scenarios.ts:587）。かつ `useRobber` は**消費コードが無い死フラグ**→陸の盗賊が通常作動。コメントは「盗賊は通常通り使用」と簡略明言（scenarios.ts:562） |
| **最長交易路 不使用** | ❌ | S7 が `useLongestRoute:false` 未設定（scenarios.ts:587）→最長交易路が通常作動 |
| **最大騎士力 不使用** | ❌ | `updateLargestArmy`（scoring.ts:259）にシナリオゲート無く game.ts:662 から無条件作動 |
...
- **[D2] S7 で 盗賊・最長交易路・最大騎士力 が無効化されていない**（scenarios.ts:587 が `useRobber/useLongestRoute` 未設定／`updateLargestArmy` にゲート無し）。公式 S7 は3要素とも不使用（7は破棄＋強奪のみ）。さらに軍船・艦隊戦闘も未実装（簡略明言）。S7 を本気で公式準拠にするなら別ゲーム級の作業。
...
- **[D5] S7 軍船・戦闘解決 未実装**（pirateIslands.ts:54-71 は常にランダム1枚）。公式は海賊強さ(目) vs 自分強さ(軍船数)で 任意1枚/なし/ランダム1枚+都市ごと1枚破棄。軍船化（騎士で起点最寄りの通常船を軍船化）も未実装。
...
| **[D2]** S7 で盗賊/最長交易路/最大騎士力 を実際にOFFにする | ⏸保留（要判断） | 基盤は実装済み・テスト済みだが **S7 シナリオでは未有効化**。下記理由。 |

### [D2] S7 を「保留」とした理由（重要）
公式 S7 は 盗賊・最長交易路・最大騎士力 を不使用にする代わりに **軍船による艦隊戦** で点を取らせる設計。
本デジタル版は軍船戦闘が未実装（[D5]）かつ盤も公式サイズでないため、上記3要素を外すと **最大騎士力(2VP)＋
最長交易路(2VP) 相当が消え、AIが「要塞制圧＋10点」に到達できず**（実測: スモークが完走しない）。
3要素のうち1つ（盗賊のみ）を外しても、騎士が連動して無効化され同様に到達不能だった。
→ ルール基盤（`useRobber`/`useLargestArmy`/`useLongestRoute` の消費）は実装・テスト済みで、
   S7 シナリオ定義に flags を付けるだけで有効化できる状態。**有効化には併せて以下のいずれかが必要**:
   (A) 軍船・艦隊戦闘の実装（[D5]）、(B) S7勝利点の見直し、(C) 盤の公式サイズ化。どれを採るか要相談。

### 引き続き未対応（次フェーズ候補）
[D5] S7軍船・戦闘 / [D6] S6海賊凍結・織物強奪 / [D7] S6村航路closed / [D8] S5港間隔 /
[D9] S8不思議要件 / [N1] S3霧ランダム数字 / [N2] S4砂漠分断 / [N3] New World制約付き生成 /
[N4] 各盤の枚数微差（要地図画像）。
