...
import { listScenarios, listVisibleScenarios, DEFAULT_RECOMMENDED_PLAYERS } from '../src/engine/scenarios';
...
describe('シナリオ選択: おすすめプレイヤー数', () => {
  it('各カードに 👥 おすすめ人数が表示され、範囲（〜）ではなく言い切る', () => {
    ...
    // 表示対象シナリオ分のカードに人数表示がある。
    expect(players.length).toBe(listVisibleScenarios().length);
    players.forEach(p => {
