# PROGRESS — カタン（航海者版＋交易と蛮族 実装中）（最終更新: 2026-07-10）
...
## 2. 完了したこと（新しい順）
- **2026-08-06 オンライン対戦の耐障害性強化（ユーザ指摘「落ちやすい／落ちると戻れない」）**:
  - **落ちない対策**: ①アプリ層ハートビート（クライアント20秒ごと `ping`、45秒無応答で自己切断→再接続。`protocol.ts` の `PING_INTERVAL_MS`/`PONG_TIMEOUT_MS`）②サーバ側 protocol ping（25秒。無応答は `terminate` して AI 代行へ）→ 「TCPは生きているが無反応」な半死接続で対局が止まる問題を解消 ③別ホスト運用時は接続前に `/health` を叩いてスリープ中の無料枠を起こす（`warmUpLanServer`。ロビーのタイムアウトも 9秒→25秒＋「起動中」表示）。
  - **落ちても戻れる対策**: 再接続の**回数上限を撤廃**（従来 MAX_RECONNECT=6・約10秒で resume 情報を捨ててホームへ＝トンネル1本で復帰不能だった）。バックオフ 0.4→15秒で無期限、`online`/`focus`/`pageshow`/`visibilitychange` で即時再試行（スマホのバックグラウンド落ち対策）。再接続バナーに「今すぐ再試行／やめる」を追加。切断中の操作は握り潰さず再接続を促す。対局中のルーム保持を 90秒→**30分**（`GAME_ROOM_KEEP_MS`）に延長。ホーム（オンラインタブ）に「▶ 前回の対局に戻る」導線。
  - **サーバから追い出されても戻れる対策（封印スナップショット）**: サーバが正本 state を **AES-256-GCM＋gzip で暗号化**して各端末へ配布（`server/roomSeal.ts`。中身は読めず改竄も検知）。再起動でルームが消えた後の `resume` には新メッセージ `restorable` を返し、端末が `restore` で封印を差し戻すとサーバが復号・検証して**対局を復元**する（`verifySnapshot`: コード一致・トークンハッシュ一致・TTL6時間）。他メンバーは通常の resume で合流。SIGTERM 時は最新スナップショットを push してから `bye` を送る（Render のデプロイ対策）。`uncaughtException`/`unhandledRejection` でプロセスを落とさない。
  - **本番の必須設定**: Render の環境変数 **`ROOM_SECRET`（長いランダム文字列）** を設定すること。未設定だと起動ごとに鍵が変わり「再起動をまたぐ復元」だけが無効（警告ログを出す）。手順は [LAN対戦.md](LAN対戦.md) §9。
  - 再接続トークンはサーバ側で**SHA-256 ハッシュのみ保持**に変更（平文はクライアントのみ。スナップショット経由の漏洩を防止）。
  - テスト: 新規 `tests/lan-resilience.test.ts`（**16件**: 封印の往復/改竄検知/鍵違い/TTL/本人確認、ping→pong、サーバ記憶消失からの復元、他人トークンでの乗っ取り拒否、ロビー猶予を過ぎても対局スロットが残る）。
- **2026-08-07 対局中のルームNo表示と「復帰コード」（ユーザ指摘「ルームNoを人に伝えられるように」）**:
  - 対局中の **☰メニュー最上段にルームNo**（タップでコピー）を表示。`updateGameNav` に `netMode` 時のみのルーム情報ブロックを追加（`copyToClipboard` は clipboard API＋execCommand フォールバックで LAN の http アクセスでも動く）。
  - **ルームNo だけでは復帰できない**（同一プレイヤーの証明＝トークンが必要）ため、No＋スロット＋トークンを1行にまとめた **復帰コード `CATAN1-…`**（`encodeResumeCode`/`decodeResumeCode`）を追加。メニューからコピーし、別端末のロビー「別の端末から対局に戻る（復帰コード）」に貼れば同じ席へ復帰できる。他人に渡すと席を乗っ取られる旨をUIに明記。
  - テスト: 新規 `tests/resume_code.test.ts`（6件・往復/先頭ゼロ保持/コピペ耐性/トークン非平文/不正入力）。実機E2E（Playwright）でメニュー表示・コピー・**別コンテキストから同じ席への復帰**を確認。
- **2026-08-06 スマホで他プレイヤーの伏せ札枚数が見えない問題（ユーザ指摘）**:
  - 縦持ちスマホ（`.panel-opp`）と横持ち四隅レイアウト（`.corner-panels`）の CSS が `.dev-card-panel` を `display:none` にしており、相手の発展カード枚数が確認できなかった。**枚数は公開情報**なのでチップを小さくして常時表示に変更。C&K の**進歩カード枚数（📜）・商品枚数（💰）**も他プレイヤー分を追加表示（中身は秘匿のまま）。盤面四隅のミニパネルにも `🃏N`（発展＋進歩）を追加。
  - テスト: 新規 `tests/opponent_cards_ui.test.ts`（3件・枚数は出るが種類は出ないことを固定）。
- **2026-07-10 ゲーム終了時のスコア表示バグ修正＋最終盤面トグル（ユーザ指摘）**:
  - **バグ**: CPU対戦・オンライン対戦で勝利した際、勝者/自分以外のプレイヤーの非公開VPカード（発展カードの勝利点）が最終スコアに反映されないことがあった。原因は3箇所: ①`engine/mask.ts` の `maskStateFor` が GAME_OVER でも「勝者と自分」だけ全公開にしており、それ以外のプレイヤー（3人以上対戦時の敗者など）はLAN配信stateで手札/発展カードが秘匿されたまま ②`renderer/ui.ts` の `buildPlayerPanel`（メインのプレイヤーパネル）の★VP表示が `isSelf` だけを見ており、`isWinner` チェック自体が漏れていたため自分以外が勝者でもVPカードが未加算 ③同ファイルの `renderMiniPanels`（盤面四隅ミニパネル）・`renderUI` の手番VP表示も同様に GAME_OVER 後の全開示に未対応。
  - **修正**: GAME_OVER になったら秘匿を維持する意味がない（勝敗確定後）という前提で、`maskStateFor` は全プレイヤーを公開に統一。UI側3箇所（`buildPlayerPanel`／`renderMiniPanels`／`renderUI`の手番VP／`buildVictoryScoreboard`）も「自分 or GAME_OVER後」で内部VP（VPカード込み）を表示するよう統一。`tests/lan-sync.test.ts` の該当テストも新仕様（敗者もGAME_OVERで公開）に更新。新規 `tests/victory_reveal_ui.test.ts`（jsdom）で「CPUの隠しVPカードがGAME_OVER後に合算表示される／ゲーム中は非公開のまま」を回帰テスト化。
  - **機能追加**: 勝利モーダルに「🗺 最終盤面を見る」ボタンを追加。押すとモーダルを一時的に隠して盤面をゆっくり見られ（感想戦用）、右下に浮動の「🏆 結果を見る」ボタンで結果画面へいつでも戻れる（`main.ts` `hideVictoryOverlayForBoardView`／`style.css` `.victory-hidden`・`#victory-reopen-btn`）。
  - 検証: vitest **1078緑**（新規3件含む）・tsc緑・実機（Playwright, CPU対戦を最後まで自動プレイ→勝利モーダル→盤面表示→結果へ復帰の一連をスクリーンショットで確認、コンソールエラー0）。
